CREATE OR REPLACE TABLE  "PROD"."WORKSPACE_GREETZ_HISTORY_MIGRATION"."DIFF_ESIV_3" AS (
...
cte_INDIVIDUALSHIPPING
 ...
 FROM 
   "PROD"."WORKSPACE_GREETZ_HISTORY_MIGRATION"."DIFF_ESIV_2" e
...
FROM "PROD"."WORKSPACE_GREETZ_HISTORY_MIGRATION"."DIFF_ESIV_2" e
    JOIN "RAW_GREETZ"."GREETZ3".orders o on e.order_id = o.id
...
SELECT ol.ID, ROW_NUMBER() OVER(PARTITION BY ol.orderid, ol.PRODUCTITEMINBASKETID ORDER BY ol.ID) AS RN
FROM "PROD"."WORKSPACE_GREETZ_HISTORY_MIGRATION"."DIFF_ESIV_2" e
    JOIN "RAW_GREETZ"."GREETZ3".orders o on e.order_id = o.id
 -- "RAW_GREETZ"."GREETZ3".orders o
	JOIN "RAW_GREETZ"."GREETZ3".orderline ol ON o.id = ol.orderid
...
-- 	 (SELECT * FROM "RAW_GREETZ"."GREETZ3".orders WHERE id = 1266779435 /*1323191458 1347309071 1336681263*/) o
  ...
  --  "RAW_GREETZ"."GREETZ3".orders o
    ...
    LEFT JOIN cte_Cards 
        ON cte_Cards.ID = ol.ID
    LEFT JOIN cte_content_0 AS co
        ON co.orderid = ol.orderid
            AND IFNULL(co.PRODUCTITEMINBASKETID, 0) = IFNULL(ol.PRODUCTITEMINBASKETID, 0)
            AND co.RN = cte_Cards.RN
...
SELECT od.order_id, count(*) as cnt, SUM(od.item_esev) as item_esev , AVG(os.ORDER_ESEV) as ORDER_ESEV -- AVG(ORDER_ISIV)
