    ...
    where PARENTCATEGORYITEMID is null
     ...
     product_type as (
         ...
         from (
                  select distinct case
                                      ...
                                      when lower(pgc2.name) = 'accessories' and lower(pgc2.child_name) = 'gadgets'
                                          then 'gadget-novelty'
                                      when lower(pgc2.name) IN ('accessories', 'blond amsterdam'
									  ...
									  when lower(pgc2.name) LIKE 'bakmix%' then 'cake'
									  ...
									  when lower(pgc2.name) IN ('book', 'kinderboekjes') then 'book'
                                      when lower(pgc2.name) LIKE '%books%' then 'book'
                                     -- when lower(pgc2.child_name) = 'bubble' then 'Bubbles'                                   
                                      when lower(pgc2.name) = 'food' AND lower(pgc2.child_name) IS NULL then 'home-gift'
									  when lower(pgc2.name) = 'food' AND lower(pgc2.child_name) = 'fruit' then 'home-gift'
		...
		-- Added 2022-04-09 - for old products:
										...
										when lower(pgc2.name) LIKE 'personalised_baby%' then 'toy-game'
										...
										when lower(pgc2.name) = 'fruitz' then 'home-gift'
...
-- 		begin of the list:
										...
										when lower(pgc2.name) = lower('Personalised_uv_printing_parcel_beverage') then 'home-gift'
